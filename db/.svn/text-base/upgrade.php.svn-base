...
function xmldb_enrol_authorize_upgrade($oldversion) {
    ...
    // Add new table for credit card address -- for use on receipt if AVS is enabled in plugin configuration
    if ($oldversion < 2011050800) {
        $table = new xmldb_table('enrol_authorize_avs');

        // Adding fields to table enrol_authorize_avs
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('orderid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        $table->add_field('ccfirstname', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('cclastname', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('ccaddress', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('cccity', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('ccstate', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('cccountry', XMLDB_TYPE_CHAR, '255', null, null, null, null);

        // Adding keys to table enrol_authorize_avs
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for enrol_authorize_avs
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // authorize savepoint reached
        upgrade_plugin_savepoint(true, 2011050800, 'enrol', 'authorize');
    }

    if ($oldversion < 2011051400) {

        // Define field receipt to be added to enrol_authorize
        $table = new xmldb_table('enrol_authorize');
        $field = new xmldb_field('receipt', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'transid');

        // Conditionally launch add field receipt
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // authorize savepoint reached
        upgrade_plugin_savepoint(true, 2011051400, 'enrol', 'authorize');
    }
